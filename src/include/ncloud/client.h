...
int set_buffered_file_write_request(request_t *req, char *filename, unsigned long int filesize, unsigned char *data, const char *storage_class, unsigned char namespace_id);
int set_cached_file_write_request(request_t *req, char *filename, unsigned long int filesize, char *cachepath, const char *storage_class, unsigned char namespace_id);
...
int set_get_append_size_request(request_t *req, const char *storage_class);
 ...
 * @return for read/write, the length of data read/file on success, -1 if failed; for others, 0 on success, -1 if failed
 **/
unsigned long int send_request(ncloud_conn_t *conn, request_t *request);

#endif //define __ZMQ_CLIENT_INTERFACE_H__
